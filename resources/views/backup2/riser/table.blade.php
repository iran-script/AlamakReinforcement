...
@section('content')
    ...
    <style>
        ...
        .status-badge.pending { background: rgba(241, 196, 15, .12); color: #b8860b; }
        .status-badge.default { background: rgba(92, 97, 97, .12); color: #5c6161; }
        .status-badge.success { background: rgba(46, 204, 113, .12); color: #1e8449; }
        .status-badge.error { background: rgba(231, 76, 60, .12); color: #c0392b; }

        /* وضعیت ناظرها */
        .status-badge.approved { background: rgba(46, 204, 113, .12); color: #1e8449; }
        .status-badge.rejected { background: rgba(231, 76, 60, .12); color: #c0392b; }
        .status-badge.need_revision { background: rgba(52, 152, 219, .12); color: #21618c; }
                    ...
                    },
                    {
                        ...
                        render: function(supervisors) {
                            ...
                            return supervisors.map(function(s) {
                                const label = supStatusLabels[s.status] || s.status;
                                return `<span class="status-badge ${s.status}">${s.name} (${label})</span>`;
                            }).join(' ');
                        }
                    },
                    {
